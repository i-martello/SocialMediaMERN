...
const ImgDetails = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await axios.get(`http://localhost:5000/imagenes/${params.id}`)
  ...
  useEffect(() => {
    (async()=>{
      const res = await axios.get(`http://localhost:5000/imagenes/${params.id}/comment`)
      setResult(res.data)
    })()
  }, [params.id, result])
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await axios.post(`http://localhost:5000/imagenes/${params.id}/comment`, formData,
